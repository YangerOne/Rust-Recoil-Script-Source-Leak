...
float Script::m249_pen() {
    // CROUCHING S + MOVING 
    if (GetAsyncKeyState(Includes::a_key) && GetAsyncKeyState(Includes::crouch_key) && Includes::m249_active == true) return 1.75;
    // STANDING S 
    if (GetAsyncKeyState(Includes::s_key) && Includes::m249_active == true) return 4.5;
    // CROUCHING A + MOVING 
    if (GetAsyncKeyState(Includes::a_key) && GetAsyncKeyState(Includes::crouch_key) && Includes::m249_active == true) return 1.75;
    // STANDING A 
    if (GetAsyncKeyState(Includes::a_key) && Includes::m249_active == true) return 4.5;
    // CROUCHING D + MOVING 
    if (GetAsyncKeyState(Includes::d_key) && GetAsyncKeyState(Includes::crouch_key) && Includes::m249_active == true) return 1.75;
    // STANDING D 
    if (GetAsyncKeyState(Includes::d_key) && Includes::m249_active == true) return 4.5;
    // CROUCHING NO KEY 
    if (GetAsyncKeyState(Includes::crouch_key) && Includes::m249_active == true) return 1;
    // STANDING NO KEY 
    else return 2;
}
...
float Script::semi_pen() {
    // STANDING S + MOVING 
    if (GetAsyncKeyState(Includes::s_key) && GetAsyncKeyState(Includes::crouch_key) && Includes::semi_active == true) return 1.2f;
    // STANDING S 
    if (GetAsyncKeyState(Includes::s_key) && Includes::semi_active == true) return 3;
    // STANDING A + MOVING 
    if (GetAsyncKeyState(Includes::a_key) && GetAsyncKeyState(Includes::crouch_key) && Includes::semi_active == true) return 1.2f;
    // STANDING A 
    if (GetAsyncKeyState(Includes::a_key) && Includes::semi_active == true) return 3;
    // STANDING D + MOVING 
    if (GetAsyncKeyState(Includes::d_key) && GetAsyncKeyState(Includes::crouch_key) && Includes::semi_active == true) return 1.2f;
    // STANDING D 
    if (GetAsyncKeyState(Includes::d_key) && Includes::semi_active == true) return 3;
    // CROUCHING NO KEY 
    if (GetAsyncKeyState(Includes::crouch_key) && Includes::semi_active == true) return 1;
    // STANDING NO KEY 
    else return 2;
}
        ...
        if (CurrentWeapon::WeaponActive() == true)
        {
            if (Includes::recoil == true && Includes::hipfire == true)
            {
                for (int i = 0; i <= CurrentWeapon::Bullet() && GetAsyncKeyState(VK_LBUTTON) & 0x8000; i++)
                {
                    ...
                    if (Includes::break_active == true)
                    {
                        smoothing((int)isMuzzleBreak((float)CurrentWeapon::Delay()), (int)isMuzzleBreak(CurrentWeapon::Control(i)), (int)CurrentWeapon::x(i), (int)CurrentWeapon::y(i));
                    }
...
double CurrentWeapon::ScopeAttachment() {
    ...
    else if (Includes::recoil == true && Includes::hipfire == true && Includes::m249_active == true)
    {
        return 1.2;
    }
}
...
double CurrentWeapon::x(int b) {
    if (Includes::randomxy_active == false) {
        ...
        if (Includes::m249_active == true) return (RecoilTables::Recoil_M249[b].x * ScopeAttachment() * BarrelAttachment() * Script::m249_pen() / 4) / Includes::game_sensitivity / Includes::XCONTROL;
        if (Includes::semi_active == true) return (RecoilTables::Recoil_Semi_r[b].x * ScopeAttachment() * BarrelAttachment() * Script::semi_pen() / 4) / Includes::game_sensitivity / Includes::XCONTROL;
    ...
    if (Includes::randomxy_active == true) {                                                                 //X RANDOMIZATION ON AND OFF
        ...
        if (Includes::m249_active == true) return ((RecoilTables::Recoil_M249[b].x * ScopeAttachment()) * BarrelAttachment()) * Script::m249_pen() / 4 / Includes::game_sensitivity / Includes::XCONTROL + rand() % (Includes::XSCALE + Includes::XSCALE) + (-Includes::XSCALE);
        if (Includes::semi_active == true) return ((RecoilTables::Recoil_Semi_r[b].x * ScopeAttachment()) * BarrelAttachment()) * Script::semi_pen() / 4 / Includes::game_sensitivity / Includes::XCONTROL + rand() % (Includes::XSCALE + Includes::XSCALE) + (-Includes::XSCALE);
...
double CurrentWeapon::y(int b) {                                                                                       //Y RANDOMIZATION ON AND OFF
    if (Includes::randomxy_active == false) {
        ...
        if (Includes::m249_active == true) return (RecoilTables::Recoil_M249[b].y * ScopeAttachment() * BarrelAttachment() * Script::m249_pen() / 4) / Includes::game_sensitivity / Includes::YCONTROL;
        if (Includes::semi_active == true) return (RecoilTables::Recoil_Semi_r[b].y * ScopeAttachment() * BarrelAttachment() * Script::semi_pen() / 4) / Includes::game_sensitivity / Includes::YCONTROL;
    ...
    if (Includes::randomxy_active == true) {
        ...
        if (Includes::m249_active == true) return ((RecoilTables::Recoil_M249[b].y * ScopeAttachment()) * BarrelAttachment()) * Script::m249_pen() / 4 / Includes::game_sensitivity / Includes::YCONTROL + rand() % (Includes::YSCALE + Includes::YSCALE) + (-Includes::YSCALE);
        if (Includes::semi_active == true) return ((RecoilTables::Recoil_Semi_r[b].y * ScopeAttachment()) * BarrelAttachment()) * Script::semi_pen() / 4 / Includes::game_sensitivity / Includes::YCONTROL + rand() % (Includes::YSCALE + Includes::YSCALE) + (-Includes::YSCALE);
...
int CurrentWeapon::Bullet() {
    ...
    else if (Includes::m249_active == true) return RecoilTables::Recoil_M249.size() - Includes::bulletscale;
    else if (Includes::semi_active == true) return RecoilTables::Recoil_Semi_r.size() - Includes::bulletscale;
...
bool CurrentWeapon::WeaponActive() {
    if (Includes::ak_active == true || Includes::lr_active == true || Includes::mp5_active == true || Includes::thompson_active == true || Includes::custom_active == true || Includes::semi_active == true || Includes::m249_active == true || Includes::py_active == true || Includes::m92_active == true || Includes::m39_active == true || Includes::revolver_active == true)
        return true;
    else
        return false;
}
...
void CurrentWeapon::RecoilBreak(int count) {
    if (Includes::semi_active == true && Includes::m92_active == true && Includes::m39_active && Includes::py_active == true && Includes::revolver_active == true) {
